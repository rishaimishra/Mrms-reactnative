...
const AssessmentCalculatorScreen = () => {
    ...
    const getSubTotal = () => {
        ...
        const formula = (propertyDistrictAmount * (getField('length', 0) ? parseFloat(getField('length')) : 1) * (getField('breadth', 0) ? parseFloat(getField('breadth')) : 1));
        ...
        //return total * productCategoryAmount;
    ...
    const getTotal = function() {
        //return this.getSubTotal() + this.getGSTTotal();
        let data=(getField('length', 0) ? parseFloat(getField('length')) : 0) * (getField('breadth', 0) ? parseFloat(getField('breadth')) : 0);
    ...
    const validate = function(val) {
        var t = val;
        val = (t.indexOf(".") >= 0) ? (t.substr(0, t.indexOf(".")) + t.substr(t.indexOf("."), 3)) : t;
    return val;  
    }
    ...
    return (
        ...
        >
            ...
            <MultiPicker
                ...
                onSelected={value => value?setField('propertyCategory', value):null}
            ...
            <View style={styles.row}>
                <View style={styles.col}>
                    <TextInput
                        label={'Length'}
                        //onChangeText={value => setField('length', value.replace(/[- #*;,.<>\{\}\[\]\\\/]/gi, ''))}
                        onChangeText={value => setField('length', validate(value))}
                        keyboardType={'numeric'}
                        length={6}
                        value={getField('length')}
                    />
                </View>
                <View style={styles.col}>
                    <TextInput
                        label={'Breadth'}
//onChangeText={value => setField('breadth', value.replace(/[- #*;,.<>\{\}\[\]\\\/]/gi, ''))}
//onChangeText={value => setField('breadth', value.toFixed(2))}
onChangeText={value => setField('breadth', validate(value))}

                        keyboardType={'numeric'}
                        length={6}
                        value={getField('breadth')}
            ...
            <Text style={styles.calcResult}>Dimension in Sq. Meters: <Text
                style={styles.calcAmount}>{getTotal()}</Text></Text>
            ...
            <MultiPicker
                ...
                onSelected={value => value?setField('valuesAdded', value):null}
            //disabled={isCompletedSaved}
            />
            ...
            {sigma_pay_url ? (<View style={{flex:1,alignItems:'center'}}><ShortButton label={'Make Payments'} onPress={handleApp} /></View>) : null}
