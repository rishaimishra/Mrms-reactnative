...
const initialState = {
	...
	isUnfinsedProperty: false,
	isDilapidatedProperty: false,
	...
	// wall_material_condition:[{percentage:0,type:''}],
	totalMeter: 1,
	...
	switch (action.type) {
		case SET_PROPERTY_FIELD: {
			...
			if (action.fieldName === "totalMeter") {
				...
				newState.digitalAddress = `${
					state.propertyPostCode
				} ${action.fieldValue.latitude.toFixed(
					4
				)} ${action.fieldValue.longitude.toFixed(4)}`;
				newState.doorLocation = `${action.fieldValue.latitude},${
					action.fieldValue.longitude
				}`;
				newState[action.fieldName] = `${action.fieldValue.latitude.toFixed(
					4
				)} ${action.fieldValue.longitude.toFixed(4)}`;
			} else {
				newState[action.fieldName] = action.fieldValue;
			...
			if (
				state.useLandLordAddress &&
				action.fieldName !== "digitalAddressLocation"
			) {
				newState[action.fieldName] = action.fieldValue;
				//   alert(newState.ownerStreetNumbernew)
				newState = {
					...newState,
					propertyStreetNumber: newState.ownerStreetNumber,
					propertyStreetNumbernew: newState.ownerStreetNumbernew,
			...
			if (
				...
				newState = {
					...
					ownerTenantTitle: state.ownerTitle,
				};
			}
		...
		case SET_PROPERTY_ADDRESS_LANDLORD: {
			if (action.useLandLordAddress) {
				return {
					...
					propertyStreetNumbernew: state.ownerStreetNumbernew,
		...
		case SET_PROPERTY: {
			...
			if (action.property.digitalAddress) {
				...
				digitalAddressLocation = `${digitalAddressArr[1]} ${
					digitalAddressArr[2]
				}`;
			}
